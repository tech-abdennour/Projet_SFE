...
function showTab(tabId) {
    ...
    for (var i = 0; i < tabs.length; i++) { tabs[i].classList.remove('active-tab'); tabs[i].style.display = 'none'; }
...
// SAUVEGARDE - RESTE SUR L'ONGLET SAUVEGARDES SANS FLASH
function saveCurrentResult() {
    ...
    .then(function(res) {
        if (res.success) {
            ...
            // Recharger la page après un court délai avec l'onglet sauvegardes
            setTimeout(function() {
                window.location.href = window.location.pathname + '?tab=sauvegardes';
            }, 1500);
...
function ajaxAction(action, id) {
    ...
    .then(function(r) { return r.json(); })
    .then(function(res) {
        if (res.success) { showToast('✅ Opération réussie !'); }
        else { showToast(res.error ? res.error : 'Erreur', true); }
        setTimeout(function() {
            window.location.search = '?tab=' + (sessionStorage.getItem('activeTab') || 'dashboard');
        }, 1000);
    });
}
